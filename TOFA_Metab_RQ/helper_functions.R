...
volcano_plot_lab_lm <- function(res, title = "", 
  ...
  res |> 
    ...
    xlim(-x_lim, x_lim) +
    ylim(y_lim) +
    geom_text_repel(data = res |>  filter(qvalue<0.1, log2FoldChange>0) |>  slice_max(order_by = log2FoldChange, n = 5), aes(label = Analyte), min.segment.length = 0, show.legend = FALSE, size = 3) +
    geom_text_repel(data = res |>  filter(qvalue<0.1, log2FoldChange<0) |>  slice_min(order_by = log2FoldChange, n = 5), aes(label = Analyte), min.segment.length = 0, show.legend = FALSE, size = 3) +
    # geom_text_repel(data = res %>% filter(!is.na(qvalue)) %>% slice_min(order_by = qvalue, n = 3), aes(label = Analyte), min.segment.length = 0, show.legend = FALSE, nudge_x = -0, nudge_y = 0.1) +
